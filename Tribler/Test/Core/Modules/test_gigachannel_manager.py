...
class TestGigaChannelManager(TriblerCoreTest):
    ...
    def test_remove_cruft_channels(self):
        ...
        mock_dl_list = [
            ...
            MockDownload(database_blob(my_chan_old_infohash), my_chan.dir_name),
            MockDownload(database_blob(my_chan.infohash), my_chan.dir_name),

            # Downloads for the updated external channel: "old ones" and "recent"
            MockDownload(database_blob(b'12331244'), chan2.dir_name),
            MockDownload(database_blob(chan2.infohash), chan2.dir_name),

            # Downloads for the unsubscribed external channel
            MockDownload(database_blob(b'1231551'), chan3.dir_name),
            MockDownload(database_blob(chan3.infohash), chan3.dir_name),
            # Orphaned download
